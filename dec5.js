voronoi(10,1,4).color(()=>cc[0]*3+1).modulate(noise(()=>cc[0]*1,()=>cc[1])).out()
...
shape(()=>cc[0]*5+2).modulate(noise(2))
  ...
  .posterize(4).modulateRepeat(shape(()=>cc[0]*20)).out(o3)
...
/// Many weird voronoi circles -- shreya likes 
shape(2, 0.001, 0.05)
  .modulateScrollY(voronoi(500,1,2),0.5).scale(100).brightness(0.001)
  .blend(o0, 0.3).mult(osc(10, 0.8, 1)
  .color(3.5, 3.0, 3.0)).blend(o0).out()

// i like this with the voronoi circles above
...
// i really liked a previous version of this where the voronoi looked like lines
...
src(o0).modulate(noise(()=>cc[2]*5)).blend(o1, 0.6).blend(o1).out()
render()
...
// to make white, just move the cc value to 0
src(o0).modulate(noise(()=>cc[3]*5)).mult(osc(()=>ccActual[0],0,2,2)).blend(o1).out(o1)

src(o0).modulate(noise(1)).mult(osc(10,1,1)).out(o1)
// yeah, with the "drops" but idk it's not supposed to be like this
//whats not supposed to be like what
/// hmm i see i see
// this are su// amazing
//the white?
//the colour
...
// Nice too 
voronoi(10).mult(osc(10,0.5,1)).rotate(()=>time).modulate(noise(10))
  .modulateKaleid(osc(1,0.5,0),10).luma(0.5)
  .saturate(2).contrast(1.2).out()


// this one is nice
voronoi(10).mult(osc(10,0.5,1)).rotate(()=>time).modulate(noise(10))
  .luma(0.5).saturate(2).contrast(1.2).out()
...
// i like this line
shape(2,0.001, 0.05).modulate(noise(()=>ccActual[3])).modulateScrollY(noise(()=>cc[0])).out()

shape(2, 0.001, 0.05)
.modulateScrollY(noise(()=>cc[0]*4,0.2)
.scale(()=>cc[1]*1.5,1,5))
  .blend(o0, 0.3)
  .mult(osc(10, 0.2, 1)
  .color(3.5, 3.0, 3.0))
...
///// grid
shape(2,0.001).repeat(1,7).add(shape(2,0.001).rotate(Math.PI/2).repeat(7)).modulate(noise(2)).mult(osc(10,0.1,1)).out()



// i lost it lol //lmao
// i was trying to think of ways to introduce xavier if we start with the p5.js dropplets
//i am waiying
//wait which visula is playing rn? i was looking for it 
osc().out()
shape(2,0.001).rotate(1.57).repeat(7).out()
...
shape(2,0.001,0.05).modulateScrollY(noise(()=>cc[0]*4,0.5)
                                    .scale(()=>cc[1]*3,1,5)).blend(o0, 0.3).mult(osc(10,0.8, 1).color(3.5, 3.0, 3.0))
.luma(1)
//.modulateRotate(voronoi(50,1))
//.diff(solid(()=>ccActual[3], ()=>ccActual[3], ()=>cActual[3]))
.invert(()=>cc[3]*128)
//.modulateKaleid(noise(()=>cc[2]*2,1 )).scale(0.2)
.blend(o0)
.out()
...
shape(2, 0.0001,0.05).modulateScrollY(noise(()=>cc[0]*4,0.5))


shape(2, 0.001, 0.05).modulateScrollY(noise(() => cc[0]*4,0.5))
  .scale(()=>cc[1]*3,1,5)
.blend(o0, 0.3).mult(osc(10, 0.8, 1)
.color(3.5, 3.0, 3.0)). luma(1)
//.modulateRotate(voronoi(50,1))
.diff(solid(()=>ccActual[3], ()=>ccActual[3], ()=>ccActual[3]))
.invert(()=>ccActual[3]).modulateKaleid(noise(()=>cc[2]*2,1 )).scale(0.2)
.blend(o0)
.out()

///this one - circlw spikes
shape().modulateRotate(gradient().scale(()=>cc[0]+0.1,0.05,0.5)
                       .modulate(noise(1,0.001))).out()

render(o0)

// this one, we are using it  -- grid multicolor lines rotating
shape(2,0.001).repeat(1,7).add(shape(2,0.001).rotate(Math.PI/2).repeat(7))
  .modulate(osc(()=>cc[4]*8)).add(src(o0).modulate(noise(()=>cc[3]*5))
  .modulateRotate(noise(0.1)).mult(osc(10,1,1).rotate(1,1)
   .blend(o0)).blend(o0)).modulateRotate(noise(1)).rotate(1,1).out(o0)

render()
src(o0).blend(src(o2),()=>cc[0]).out(o1)
